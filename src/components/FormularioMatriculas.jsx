...
export const FormularioMatriculas = ({ matricula }) => {
	...
	const handleSubmit = async (e) => {
		...
		if (matricula?._id) {
			await axios.put(
				`${import.meta.env.VITE_BACKEND_URL}/matricula/${matricula?.id}`,
				form,
				{
					headers: {
						method: "PUT",
	...
	return (
		<form onSubmit={handleSubmit}>
			...
			<div>
				...
				<select size={materias.length} multiple
					id="materia"
					type="text"
					className="border-2 w-full p-2 mt-2 placeholder-gray-400 rounded-md mb-5"
					placeholder="Materia"
					...
					onChange={(event) => { handleChange(event);
						if (!materiaSelect.includes(event.target.value)) {
							setMateriaSelect([...materiaSelect, event.target.value]);
						}
					}}
				>
